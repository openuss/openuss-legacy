 ...
 * performance), please contact me at the email below. The latest version of
...
public class HtmlInputFilter {
	 ...
	 */
	static final boolean ALWAYS_MAKE_TAGS = true;

	/**
	 * flag determine whether comments are allowed in input String.
	 */
	static final boolean STRIP_COMMENTS = true;

	/** regex flag union representing /si modifiers in php * */
	static final int REGEX_FLAGS_SI = Pattern.CASE_INSENSITIVE | Pattern.DOTALL;
	...
	public synchronized String filter(String input) {
		...
		s = escapeComments(s);
		logger.debug("     escapeComments: " + s);

		s = balanceHTML(s);
		logger.debug("        balanceHTML: " + s);

		s = checkTags(s);
		logger.debug("          checkTags: " + s);

 ...
 * @author Ingo Dueppe 
 */
public class UniversityType implements java.io.Serializable {
	/**
	 * The serial version UID of this class. Needed for serialization.
	 */
	private static final long serialVersionUID = 7234562923167281047L;

	public static final UniversityType MISC = new UniversityType(-1);

	public static final UniversityType UNIVERSITY = new UniversityType(0);

	public static final UniversityType COMPANY = new UniversityType(2);

	private Integer value;

	private UniversityType(Integer value) {
		this.value = value;
	}
	...
	/**
	 * Creates an instance of UniversityType from <code>value</code>.
	 * 
	 * @param value
	 *            the value to create the UniversityType from.
	 */
	public static UniversityType fromInteger(Integer value) {
		final UniversityType typeValue = values.get(value);
		if (typeValue == null) {
	...
	public int compareTo(Object that) {
		return (this == that) ? 0 : this.getValue().compareTo(((UniversityType) that).getValue());
	}
	...
	public boolean equals(Object object) {
		return (this == object)
				|| (object instanceof UniversityType && ((UniversityType) object).getValue().equals(this.getValue()));
	}
	...
	private Object readResolve() throws java.io.ObjectStreamException {
		return UniversityType.fromInteger(this.value);
	}

	private static final Map<Integer, UniversityType> values = new HashMap<Integer, UniversityType>(3, 1);
	 ...
	 */
	static {
		values.put(MISC.value, MISC);
		literals.add(MISC.value);
		names.add("MISC");
		values.put(UNIVERSITY.value, UNIVERSITY);
		literals.add(UNIVERSITY.value);
		names.add("UNIVERSITY");
		values.put(COMPANY.value, COMPANY);
		literals.add(COMPANY.value);
		names.add("COMPANY");

...
public class MailingListFeed extends AbstractFeed {
	...
	private transient CourseMailingListService courseMailingListService;
	...
	private FeedWrapper buildFeedArray(CourseInfo course) {
		...
		List mails = getCourseMailingListService().getMails(course);
		...
		if (mails != null && mails.size() != 0) {
			...
			while (i.hasNext()) {
				...
				mailDetail = getCourseMailingListService().getMail(mailInfo);
				this.addEntry(entries, mailDetail.getSubject(), link, mailDetail.getSendDate(), mailDetail.getText(),
						course.getName(), course.getName());
			}
			mailDetail = getCourseMailingListService().getMail((MailInfo) mails.get(0));
		...
		feedWrapper.setWriter(this.convertToXml("["+i18n("rss_mailinglist", null, new Locale(getSecurityService().getCurrentUser().getLocale()))+"] "+course.getName(),

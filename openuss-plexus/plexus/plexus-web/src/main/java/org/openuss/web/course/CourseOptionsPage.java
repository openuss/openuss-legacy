...
/*	TODO what about this below?
	...
	public void preprocess() {
		if (course != null) {
			logger.debug("---------------> refreshing course.");
...
*/
	...
	public String saveOptions() throws LectureException {
		...
		CourseInfo courseOld = getCourseService().getCourseInfo(course);
		if (courseOld.getAccessType()==AccessType.APPLICATION&&course.getAccessType()!=AccessType.APPLICATION){
			getCourseService().removeAspirants(courseOld);
		}
		lectureService.persist(course);
	...
	public void processAccessTypeChanged(ValueChangeEvent event) {
		...
		course.setAccessType((AccessType)accessType);
	}

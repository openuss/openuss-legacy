 ...
 * @author Ingo Dueppe
 */
@Bean(name = "views$secured$enrollment$enrollmentnews", scope = Scope.REQUEST)
@View
public class EnrollmentNewsPage extends AbstractEnrollmentPage {

	private static final Logger logger = Logger.getLogger(EnrollmentNewsPage.class);
	...
	private class NewsDataProvider extends AbstractPagedTable<NewsItemInfo> {
		...
		public DataPage<NewsItemInfo> getDataPage(int startRow, int pageSize) {
			if (page == null) {
				List<NewsItemInfo> news = newsService.getNewsItems(enrollment);
	...
	public String addNewsItem() {
		...
		newsItem.setPublisherName(enrollment.getFaculty().getName());
		
		setSessionBean(Constants.NEWS_SELECTED_NEWSITEM, newsItem);
		return Constants.ENROLLMENT_NEWS_EDIT_PAGE;
	}
	...
	public List<NewsItem> getNewsItems() {
		logger.debug("getting newsitems for enrollment " + enrollment);

...
//		create AuthenticationDomainInfo object
	...
	private void cleanDatabase() {
		ldapServerDao.remove(ldapServerDao.loadAll());
		userDnPatternDao.remove(userDnPatternDao.loadAll());
		authenticationDomainDao.remove(authenticationDomainDao.loadAll());
		attributeMappingDao.remove(attributeMappingDao.loadAll());
		roleAttributeKeyDao.remove(roleAttributeKeyDao.loadAll());
		commit();
	}
	...
	public void testRoleAttributeKey() {
		service = getLdapConfigurationService();
		int originalRoleAttributes = service.getAllRoleAttributeKeys().size();
		int originalAttributeMappings = service.getAllAttributeMappings().size();
		...
		assertEquals(originalRoleAttributes + 2, allRoleAttributeKeys.size());
		...
		assertEquals(originalAttributeMappings + 1 , allAttributeMappings.size());
		...
		assertEquals(1 , allRoleAttributeKeysMap1Count);
		...
		assertEquals(2 , allRoleAttributeKeysMap1.size());
	...
	public void testIsValid() {
		...
		assertFalse(service.isValidRoleAttributeKey(key1));
		
		UserDnPatternInfo pattern1 = service.createUserDnPattern(createUserDnPatternInfoDummy());
		assertFalse(service.isValidUserDnPattern(pattern1));
	...
	public void testGetter() {
		...
		AuthenticationDomainInfo domain1 = createAuthenticationDomainInfoDummy(map1);
		domain1.setName("Domain 1");
		service.createDomain(domain1);
		
		AuthenticationDomainInfo domain2 = createAuthenticationDomainInfoDummy(map2);
		domain2.setName("Domain 2");
		service.createDomain(domain2);
		...
		assertEquals(2, roleAttributeKeys.size());
		for (RoleAttributeKeyInfo roleAttributeKey : roleAttributeKeys) {
			assertEquals(1, roleAttributeKey.getAttributeMappingIds().size());
		}
		assertEquals(map1.getId(), roleAttributeKeys.get(0).getAttributeMappingIds().get(0));
		assertEquals(map2.getId(), roleAttributeKeys.get(1).getAttributeMappingIds().get(0));

		RoleAttributeKeyInfo findKey = service.getRoleAttributeKeyById(key1.getId());
		assertEquals("Key 1", findKey.getName());
		findKey = service.getRoleAttributeKeyById(key2.getId());
		assertEquals("Key 2", findKey.getName());
		
		AttributeMappingInfo findMap = service.getAttributeMappingById(map1.getId());
		assertEquals("Mapping 1", findMap.getMappingName());
		findMap = service.getAttributeMappingById(map2.getId());
		assertEquals("Mapping 2", findMap.getMappingName());
		
		AuthenticationDomainInfo findDomain = service.getDomainById(domain1.getId());
		assertEquals("domain 1", findDomain.getName());
		findDomain = service.getDomainById(domain2.getId());
		assertEquals("domain 2", findDomain.getName());
		
		UserDnPatternInfo findPattern = service.getUserDnPatternById(pattern1.getId());
		assertEquals("Pattern 1", findPattern.getName());
		findPattern = service.getUserDnPatternById(pattern2.getId());
		assertEquals("Pattern 2", findPattern.getName());
		
		LdapServerInfo findServer = service.getLdapServerById(server1.getId());
		assertEquals("Server 1", findServer.getDescription());
		findServer = service.getLdapServerById(server2.getId());
		assertEquals("Server 2", findServer.getDescription());
...
//		*********************** create LdapServerInfo object ********************
		...
		try {
			service.saveLdapServer(server);
		} catch (LdapConfigurationServiceException notExpected) {
			fail("Should NOT have raised any LdapConfigurationServiceException: "+ notExpected.getMessage());
		}
...
//		check delete method
	...
	public void testGetEnabledLdapServerConfigurations() {
		...
		assertEquals(2, service.getEnabledLdapServerConfigurations().size());		

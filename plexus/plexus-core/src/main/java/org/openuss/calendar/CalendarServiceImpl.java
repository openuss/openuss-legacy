...
public class CalendarServiceImpl extends
	 ...
	 */
	protected void handleCreateUserCalendar(
			org.openuss.security.UserInfo userInfo) throws java.lang.Exception {
	...
	protected void handleCreateAppointment(
		...
		// add appointment to the calendar to which the appointment originally
		// belongs to

		calendar.addOwnAppointment(app);

		// add appointment to all calendars which have subscribed to the source
		// calendar

		Set<Calendar> subscribedCals = calendar.getSubscribedCalendars();

		if (!subscribedCals.isEmpty()) {
			for (Calendar cal : subscribedCals) {
				cal.addLinkedAppointment(app);
			}
		}
	...
	protected void handleDeleteAppointment(
			org.openuss.calendar.AppointmentInfo singleAppointment,
			org.openuss.calendar.CalendarInfo calendar)
			throws java.lang.Exception {
		// if (singleAppointment.getId() == null)
		// throw new Exception("Appointment does not exist");
		// Appointment app =
		// getAppointmentDao().load(singleAppointment.getId());
	...
	protected void handleCreateSerialAppointment(
		...
		// if
		// (serialAppointment.getRecurrenceType().equals(RecurrenceType.daily))
		// {

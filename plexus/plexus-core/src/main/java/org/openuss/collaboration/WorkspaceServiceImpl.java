...
//		new DefaultDomainObject (workspaceInfo.getDomainId ()) ;
		getSecurityService().setPermissions(getSecurityService().getUserObject(getSecurityService().getCurrentUser()),
				workspaceEntity, LectureAclEntry.OGCRUD);
	}

	@SuppressWarnings("unchecked")
	// NOPMD by Administrator on 13.03.08 13:11
	@Override
	...
	// NOPMD by Administrator on 13.03.08 13:11
	@Override
	protected List handleFindWorkspacesByDomainAndUser(Long domainId, final UserInfo user) {
		Validate.notNull(domainId, "domainId cannot be null.");
		...
		final Predicate containsUser = new Predicate() {
			public boolean evaluate(Object object) {
				return (object instanceof User) && ObjectUtils.equals(((User) object).getId(), user.getId());
			}
		};
		for (Workspace ws : workspaces) {
			if (CollectionUtils.exists(ws.getUser(), containsUser)) {

    ...
    extends org.openuss.internalMessage.InternalMessageServiceBase
{
    ...
    {
    	User user = getUserDao().load(getSecurityService().getCurrentUser().getId());
        ...
        throws InternalMessageServiceException
    {
		...
		if (messageInfo.getSenderId() != null) {
			...
			sender = getInternalMessageCenterDao().findByUser(getUserDao().load(getSecurityService().getCurrentUser().getId()));
		}
		if (sender == null) {
			sender = getInternalMessageCenterDao().create(getUserDao().load(getSecurityService().getCurrentUser().getId()));
		}

...
public class ActiveDirectoryBindAuthenticator extends AbstractLdapAuthenticator {
   ...
   public ActiveDirectoryBindAuthenticator(InitialDirContextFactory initialDirContextFactory) {
       ...
       userDetailsMapper.setRoleAttributes(new String[]{"memberOf"});
   }
   ...
   private LdapUserDetails bindWithDn(String userDn, String username, String password) {
       // MS Active Directory needs username instead of userDn as javax.naming.Context.SECURITY_PRINCIPAL
	   LdapTemplate template = new ActiveDirectoryLdapTemplate(getInitialDirContextFactory(), username, password);
       try {

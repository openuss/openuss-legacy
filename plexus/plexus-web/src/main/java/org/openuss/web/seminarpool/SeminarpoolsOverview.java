...
@Bean(name = "views$public$institute$seminarpoolsbyuniversityoverview", scope = Scope.REQUEST)
@View
public class SeminarpoolsOverview extends BasePage {

	protected static final Logger logger = Logger.getLogger(SeminarpoolsOverview.class);
	...
	public Boolean getBookmarked() {
		try {
			SeminarpoolInfo currentSeminarpool = currentSeminarpool();
			return desktopService2.isSeminarpoolBookmarked(currentSeminarpool.getId(), user.getId());
		} catch (Exception e) {
			logger.error(e);
		}
	...
	public String removeShortcut() {
		try {
			SeminarpoolInfo currentSeminarpool = currentSeminarpool();
			desktopService2.unlinkInstitute(desktopInfo.getId(), currentSeminarpool.getId());
		} catch (Exception e) {

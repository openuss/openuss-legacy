...
@Bean(name = "views$public$seminarpool$seminarpoolsbydepartmentandstatusoverview", scope = Scope.REQUEST)
@View
public class SeminarpoolsByDepartmentAndStatusOverview extends BasePage {

	protected static final Logger logger = Logger.getLogger(SeminarpoolsByDepartmentAndStatusOverview.class);
	...
	public DataPage<SeminarpoolInfo> fetchDataPage(int startRow, int pageSize) {
		...
		if (dataPage == null) {
			...
			List<SeminarpoolInfo> seminarpoolList = new ArrayList<SeminarpoolInfo>(getSeminarpoolAdministrationService().findSeminarpoolsByDepartmentAndStatus(departmentInfo.getId(), SeminarpoolStatus.REGISTRATIONPHASE));

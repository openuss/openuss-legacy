		...
		if (dataPage == null) {
			...
			if (periodInfo != null) {
				...
				if (periodInfo.getId().longValue() == Constants.COURSES_ALL_PERIODS) {
					courseList = getCourseService().findAllCoursesByDepartmentAndEnabled(departmentInfo.getId(), true);
				} else if (periodInfo.getId().longValue() == Constants.COURSES_ALL_ACTIVE_PERIODS){
					courseList = getCourseService().findAllCoursesByDepartmentAndActivePeriodsAndEnabled(departmentInfo.getId(), true);
				} else {
					courseList = getCourseService().findAllCoursesByDepartmentAndPeriodAndEnabled(departmentInfo.getId(), periodInfo.getId(), true);
				}			
			}
			sort(courseList);

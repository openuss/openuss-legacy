...
@View
public class GroupsJoinPage extends BasePage{
	...
	private List<UserGroupInfo> getGroups() {
		if (groups == null) {
			...
			logger.debug("USER GROUPS: " + groupService.getGroupsByUser(user.getId()).size() + ", " + groupService.getGroupsByUser(user.getId()));

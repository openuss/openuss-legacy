...
public class FLGFileUtility {
    ...
    public static File createNewFile(String fileNamePrefix, String fileNameSuffix, File directory) {
        ...
        while (new File(directory, fileNamePrefix + fileNameNumber + fileNameSuffix).exists())
            fileNameNumber++;
    ...
    public static void cleanDirectory(File directory, File[] fileList) {
        ...
        // compare fileList to directoryContent
        // and remove allowed files from directoryContent (will not be deleted)
        // Subdirectories of allowed directories will not be deleted
        if (fileList != null) {
            for (int i = 0; i < fileList.length; i++) {
                File file = fileList[i];
                File parent = file.getParentFile();
                directoryContent.remove(file.getPath());
                // If fileList contains a directory, all contained files will be preserved
                if (file.isDirectory()) {
                ...
                //	upper directory will be preserved
        ...
        // now remove remaining entries from directoryContent
        for (int i = 0; i < directoryContent.size(); i++) {
    ...
    public static Vector getSubdirectories(File directory) {
        ...
        File[] content = directory.listFiles(); // Dateien und Verzeichnisse
        if (content != null) {
    ...
    public static Vector readDirectoryContent(File directory) {
        Vector contentVector = new Vector();
        File[] content = directory.listFiles(); // files and directories
        if (content != null) {
            for (int i = 0; i < content.length; i++) {
                if (content[i].isDirectory()) {
    ...
    public static File[] bubbleSortFiles(File[] unsortedFiles, boolean ascending) {
        ...
        // ascending order
            for (int i = 0; i < unsortedFiles.length - 1; i++) {
                for (int j = 1; j < unsortedFiles.length - 1 - i; j++)
                    if ((unsortedFiles[j + 1].getName()).compareToIgnoreCase(unsortedFiles[j].getName()) < 0) {
            ...
            for (int i = unsortedFiles.length - 2; i >= 0; i--) {
                for (int j = unsortedFiles.length - 2 - i; j >= 0; j--)
                    if ((unsortedFiles[j + 1].getName()).compareToIgnoreCase(unsortedFiles[j].getName()) > 0) {
                        File swap = unsortedFiles[j];
                        unsortedFiles[j] = unsortedFiles[j + 1];                            
                        unsortedFiles[j + 1] = swap;
    ...
    public static DefaultTreeModel loadDirectoryContent(File directory) {
        ...
        // sort files because listFiles does not garantees ar correct order
    ...
    public static String getExtension(String filename) {
        // seek from end of name the first occurence of "."
        String ext = "";
        ...
        if (directory.isDirectory()) {
            ...
            for (int i = 0; i < length; i++) {
                if (files[i].isDirectory())
                    result += fileCount(files[i], fileFilter);
                else {
    ...
    public static long size(File directory) {
        ...
        if (directory.isDirectory()) {
            ...
            int length = files.length;
            for (int i = 0; i < length; i++) {
                if (files[i].isDirectory()) result += size(files[i]);
                else
     ...
     * @return										true, if successful.
     */
    public static boolean copyFileStructure(File sourceDirectory, File targetDirectory, boolean withSubDirectories,
     ...
     * @return										true, if successful.
     */
    private static boolean copyFileStructureRecursive(File sourceFile, File targetDirectory, boolean withSubDirectories,
        StringBuffer errorLog) {
            boolean result = true;
            if (abortCopy) return false;
                ...
                if (withSubDirectories) {
                    ...
                    if (sourceFile.list() != null && sourceFile.list().length > 0) {
                        ...
                        for (int i = 0; i < srcDirContent.length; i++) {
                            ...
                            result = result & copyFileStructureRecursive(srcDirContent[i], newDir,
                                withSubDirectories, errorLog);
                        }
                    }
                }
                else {
                    if (sourceFile.list() != null && sourceFile.list().length > 0) {
                        ...
                        for (int i = 0; i < srcDirContent.length; i++) {
                            if (abortCopy) return false;
                            if (!result) return result;
                            result = result & copyFileStructure(srcDirContent[i], targetDirectory,
                                withSubDirectories, errorLog);
                        }
                    }
                }
    ...
    public static boolean delete(File file, StringBuffer errorLog) {
        ...
        if (file.isDirectory()) {
            ...
            if (file.list() != null && file.list().length > 0) {
                for (int i = 0; i < fileDirContent.length; i++)
                    result = result & delete(fileDirContent[i], errorLog);
            }
            result = file.delete();
        }
        else {

package freestyleLearningGroup
.freestyleLearning
.learningUnitViewManagers
.glossary
.elementsContentsPanel;
...
public class FLGMemoryContentPanel
extends FSLAbstractLearningUnitViewElementContentPanel {
	...
	public void init(
		...
		m_internationalization =
			new FLGInternationalization(
					"freestyleLearningGroup.freestyleLearning.learningUnitViewManagers.glossary.elementsContentsPanel.internationalization",
		...
		learningUnitViewManager.addLearningUnitViewListener(
				new FLGMemoryContentPanel_Adapter());
	}
	
	public void saveUserChanges() {
		((FLGGlossaryManager) learningUnitViewManager).setModifiedByMemory(
				false);
	}
	...
	public boolean isModifiedByUserInput() {
		return ((FLGGlossaryManager) learningUnitViewManager)
		.isModifiedByMemory();
	}
	
	protected void buildIndependentUI() {
		add(m_memoryStandardPanel, "standardPanel"); //$NON-NLS-1$
		add(m_memoryStatisticPanel, "statisticPanel"); //$NON-NLS-1$
		((CardLayout) getLayout()).show(this, "startPanel"); //$NON-NLS-1$
	}
	...
	protected void start() {
		// Sanity...
		if (learningUnitViewElementsManager
				.getAllLearningUnitViewElementIds()
				.length
				== 0) {
			FLGOptionPane.showMessageDialog(m_internationalization.getString("FLGMemoryContentPanel.noElementsMsg"), //$NON-NLS-1$
					m_internationalization.getString("FLGMemoryContentPanel.sessionNotPossibleMsg"), //$NON-NLS-1$
		...
		((FLGGlossaryManager) learningUnitViewManager).setModifiedByMemory(
				true);
		if (dialog.getStrategy() == MemoryStartDialog.SINCE_STRATEGY)
			createListByDate(dialog.getNotRepeatedSince());
		else
			// Here we have the category strategy...
			createListByCategory(dialog.getCategory());
		Collections.shuffle(m_currentSession);
		// We have to check if the list is empty
		if (m_currentSession.size() == 0) {
			FLGOptionPane.showMessageDialog(m_internationalization.getString("FLGMemoryContentPanel.selectionContainsNoElementsMsg"), //$NON-NLS-1$
					m_internationalization.getString("FLGMemoryContentPanel.sessionNotPossibleMsg"), //$NON-NLS-1$
					FLGOptionPane.INFORMATION_MESSAGE);
			return;
		}
		m_currentIndex = -1;
		// Check wether the content or the title will be shown first
		if (dialog.isContentFirst())
			m_memoryStandardPanel.setShowMode(
					MemoryStandardPanel.CONTENT_FIRST);
		else
			m_memoryStandardPanel.setShowMode(MemoryStandardPanel.TITLE_FIRST);
		// Start!
		next();
	}
	...
	protected void uncover() {
		...
		learningUnitViewManager.fireLearningUnitViewEvent(
				new FLGMemoryEvent(
						FLGMemoryEvent.MEMORY_UNCOVERED,
						((FLGGlossaryElement) m_currentSession.get(m_currentIndex))
						.getId()));
	}
	
	protected void moveUp() {
		FLGGlossaryElement currentElement =
			(FLGGlossaryElement) m_currentSession.get(m_currentIndex);
	...
	protected void showStatistics() {
		...
		((CardLayout) getLayout()).show(this, "statisticPanel"); //$NON-NLS-1$
	}
	
	protected void next() {
		// Check if we had the last element
		if (m_currentIndex++ == m_currentSession.size() - 1) {
			showStatistics();
			learningUnitViewManager.fireLearningUnitViewEvent(
					new FLGMemoryEvent(
							FLGMemoryEvent.MEMORY_SESSION_FINISHED,
							null));
			return;
		}
		m_lastStartTime = System.currentTimeMillis();
		try {
			m_memoryStandardPanel.setGlossaryElement(
					(FLGGlossaryElement) m_currentSession.get(m_currentIndex));
			learningUnitViewManager.fireLearningUnitViewEvent(
					new FLGMemoryEvent(
							FLGMemoryEvent.MEMORY_COVERED,
							((FLGGlossaryElement) m_currentSession.get(m_currentIndex))
							.getId()));
		} catch (Exception e) {
			// This is a workaround as some elements cannot be found
	...
	protected void stop() {
		...
		learningUnitViewManager.fireLearningUnitViewEvent(
				new FLGMemoryEvent(FLGMemoryEvent.MEMORY_STOPPED));
		
	}
	
	private void createListByCategory(int category) {
		String elementIds[] =
			learningUnitViewElementsManager.getAllLearningUnitViewElementIds();
		m_currentSession.clear();
		for (int i = 0; i < elementIds.length; i++) {
			FLGGlossaryElement element =
				(
						FLGGlossaryElement) learningUnitViewElementsManager
						.getLearningUnitViewElement(
								elementIds[i],
								false);
			if (!element.getFolder()
					&& element.getCategory() == category
					&& element.getMemoryUsable())
				m_currentSession.add(element);
		}
	}
			...
			FLGGlossaryElement element =
				(
						...
						.getLearningUnitViewElement(
								...
								false);
			if (!element.getFolder()
					&& element.getLastTryAsDate() != null
					&& element.getLastTryAsDate().before(date)
					&& element.getMemoryUsable())
				m_currentSession.add(element);
		}
	}
	
	protected class FLGMemoryContentPanel_Adapter

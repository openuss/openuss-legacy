/*
 * FreeMindget - A Program for creating and viewing Mindmaps Copyright (C)
 ...
 */
/* $Id: MindMapMapModel.java,v 1.5 2004/09/01 04:46:44 jbrenni Exp $ */
...
import freestyleLearningGroup.freestyleLearning.learningUnitViewManagers.mindmaps.freemind.modes.LinkRegistryAdapter;
...
import freestyleLearningGroup.freestyleLearning.learningUnitViewManagers.mindmaps.freemind.modes.MindMapLinkRegistry;
...
public class MindMapMapModel extends MapAdapter {
    ...
    public MindMapMapModel(FreeMindMain frame, String title) {
        this(new MindMapNodeModel(title,
                frame), frame);
    }

    public MindMapMapModel(MindMapNodeModel root, FreeMindMain frame) {
        super(frame);
        lockManager = frame.getProperty("experimental_file_locking_on").equals(
                "true") ? new LockManager() : new DummyLockManager();
        ...
        boolean filesShouldBeDeletedAfterShutdown = Tools.safeEquals(getFrame()
                .getProperty("delete_automatic_saves_at_exit"), "true");
        ...
        timerForAutomaticSaving.schedule(new doAutomaticSave(this,
                numberOfTempFiles, filesShouldBeDeletedAfterShutdown,
    ...
    public String getRestoreable() {
        return getFile() == null ? null : "MindMap:"
                + getFile().getAbsolutePath();
    }

    //  All these methods do redisplay, because they are offered to controller
    // for use.
    ...
    public void setArrowLinkColor(MindMapNode source,
            MindMapArrowLinkModel arrowLink, Color color) {
    ...
    public void setFontFamily(MindMapNodeModel node, String fontFamily) {
        ...
        node.setFont(getFrame().getController().getFontThroughMap(
                new Font(fontFamily, node.getFont().getStyle(), node.getFont()
                        .getSize())));
        nodeChanged(node);
    }
    ...
    public void increaseFontSize(MindMapNodeModel node, int increment) {
        node.estabilishOwnFont();
        int currentFontSize = node.getFont().getSize();
        if ((increment < 0 && currentFontSize > 6) || (increment > 0 && currentFontSize < 72)) {
    ...
    public boolean saveHTML(MindMapNodeModel rootNodeOfBranch, File file) {
        ...
        try {
            ...
            String htmlExportFoldingOption = getFrame().getProperty(
                    "html_export_folding");
            boolean writeFoldingCode = (htmlExportFoldingOption
            ...
            rootNodeOfBranch.saveHTML(fileout, "1", 0, /* isRoot */true, /* treatAsParagraph */
                    true, /* depth */1);
     ...
     * @throws Exception,
     *             when the locking failed for other reasons than that the file
     *             is being edited.
     */
        ...
        if (!file.canWrite()) {
            ...
            try {
                ...
                if (lockingUser != null) {
                    getFrame().getController().informationMessage(
                            Tools.expandPlaceholders(
                                    getText("map_locked_by_open"), file
                                            .getName(), lockingUser));
                    readOnly = true;
                } else {
                    readOnly = false;
                ...
                getFrame().getController().informationMessage(
                        Tools.expandPlaceholders(
                                getText("locking_failed_by_open"), file
                                        .getName()));
                readOnly = true;
            }
        }
    ...
    MindMapNodeModel loadTree(File file) throws XMLParseException, IOException {
        ...
        try {
            mapElement
                    .parseFromReader(new BufferedReader(new FileReader(file)));
        } catch (Exception ex) {
    ...
    public void splitNode(MindMapNode node, int caretPosition, String newText) {
        ...
        String newContent = currentText.substring(caretPosition, currentText
                .length());
        MindMapNodeModel upperNode = new MindMapNodeModel(currentText
                .substring(0, caretPosition), getFrame());
    ...
    public void joinNodes() {
        MindMapNode selectedNode = getFrame().getView().getSelected()
                .getModel();
        ArrayList selectedNodes = getFrame().getView()
        ...
        for (Iterator it = selectedNodes.iterator(); it.hasNext();) {
            ...
            if (node.hasChildren()) {
                JOptionPane.showMessageDialog(node.getViewer(),
                        getText("cannot_join_nodes_with_children"), "FreeMindAdapter",
        ...
        if (folder.isDirectory()) {
            ...
            for (int i = 0; i < list.length; i++) {
                if (list[i].isDirectory()) {
                    // Insert a new node
                    MindMapNodeModel node = new MindMapNodeModel(list[i]
                            .getName(), getFrame());
            ...
            for (int i = 0; i < list.length; i++) {
                if (!list[i].isDirectory()
                    ...
                    favoritesFound = true;
                    try {
                        MindMapNodeModel node = new MindMapNodeModel(Tools
                                .removeExtension(list[i].getName()), getFrame());
    ...
    private MindMapNodeModel pasteXMLWithoutRedisplay(String pasted,
            MindMapNode target) throws XMLParseException {
        return pasteXMLWithoutRedisplay(pasted, target, /* asSibling= */false);
    }

    private MindMapNodeModel pasteXMLWithoutRedisplay(String pasted,
            MindMapNode target, boolean asSibling) throws XMLParseException {
    ...
    private MindMapNode pasteStringWithoutRedisplay(String textFromClipboard,
        ...
        for (int i = 0; i < textLines.length; ++i) {
            ...
            MindMapNodeModel node = new MindMapNodeModel(visibleText,
                    getFrame());
            if (textLines.length == 1) {
                pastedNode = node;
            }
        ...
        if (asSibling) {
            for (Iterator i = parent.childrenUnfolded(); /* children.iterator() */i
                    .hasNext();) {
                insertNodeIntoNoEvent((MindMapNode) i.next(), realParent,
                        asSibling);
            }
        ...
        try {
            ...
            if (t.isDataFlavorSupported(MindMapNodesSelection.fileListFlavor)) {
                ...
                for (ListIterator it = fileList.listIterator(); it.hasNext();) {
                    ...
                    MindMapNodeModel node = new MindMapNodeModel(
                            file.getName(), getFrame());
                ...
                nodeStructureChanged(asSibling ? target.getParent() : target);
                    ...
                    .isDataFlavorSupported(MindMapNodesSelection.mindMapNodesFlavor)) {
                ...
                for (int i = 0; i < textLines.length; ++i) {
                    ...
                    .isDataFlavorSupported(MindMapNodesSelection.htmlFlavor)) {
                ...
                for (int i = 1; i < links.length; i++) {
                    ...
                    String textWithHtml = links[i].replaceAll("^[^>]*>", "")
                            .replaceAll("</[aA]>[\\s\\S]*", "");
                    String text = Tools.toXMLUnescapedText(textWithHtml
                            .replaceAll("\\n", "").replaceAll("<[^>]*>", "")
                            .trim());
                    if (text.equals("")) {
                        text = link;
                        ...
                        try {
                            ...
                            if (referenceURL == null && !baseUrlCanceled) {
                                String referenceURLString = JOptionPane
                                        .showInputDialog(getText("enter_base_url"));
                    ...
                    if (linkURL != null) {
                        if (links.length == 2 & pastedNode != null) {
                            // pastedNode != null iff the number of pasted lines
                            // is one
                            // The firts element in links[] array is never a
                            // link, therefore
                        ...
                        if (linkParentNode == null) {
                            linkParentNode = new MindMapNodeModel("Links",
                                    getFrame());
                            // Here we cannot set bold, because
                            // linkParentNode.font is null
                            insertNodeInto(linkParentNode, target);
                            linkParentNode.setBold(true);
                        }
                        MindMapNodeModel linkNode = new MindMapNodeModel(text,
                                getFrame());
    ...
    public String getXMLRepresentation() throws IOException {
        String xmlRepresentation;
        StringWriter stringWriter = new StringWriter();
        stringWriter.write("<map>\n");
        ((MindMapNodeModel) getRoot()).save(stringWriter, this);
        stringWriter.write("</map>\n");

 ...
 */
/* $Id: MindMapController.java,v 1.6 2004/09/01 04:46:45 jbrenni Exp $ */
...
public class MindMapController extends ControllerAdapter {
    ...
    protected MindMapNode newNode() {
        return new MindMapNodeModel("" // getText("new_node") (PN) nicer when
                // created
                , getFrame());
    }
    ...
    public JPopupMenu getPopupForModel(java.lang.Object obj) {
        if (obj instanceof MindMapArrowLinkModel) {
            ...
            arrowLinkPopup
                    .add(new RemoveArrowLinkAction(link.getSource(), link));
            arrowLinkPopup
                    .add(new ColorArrowLinkAction(link.getSource(), link));
            arrowLinkPopup.addSeparator();
            /* The arrow state as radio buttons: */
            JRadioButtonMenuItem itemnn = new JRadioButtonMenuItem(
                    new ChangeArrowsInArrowLinkAction("none",
                            "images/arrow-mode-none.gif", link.getSource(),
                            link, false, false));
            arrowLinkPopup.add(itemnn);
            JRadioButtonMenuItem itemnt = new JRadioButtonMenuItem(
                    new ChangeArrowsInArrowLinkAction("forward",
                            "images/arrow-mode-forward.gif", link.getSource(),
                            link, false, true));
            arrowLinkPopup.add(itemnt);
            JRadioButtonMenuItem itemtn = new JRadioButtonMenuItem(
                    new ChangeArrowsInArrowLinkAction("backward",
                            "images/arrow-mode-backward.gif", link.getSource(),
                            link, true, false));
            arrowLinkPopup.add(itemtn);
            JRadioButtonMenuItem itemtt = new JRadioButtonMenuItem(
                    new ChangeArrowsInArrowLinkAction("both",
                            "images/arrow-mode-both.gif", link.getSource(),
                            link, true, true));
            ...
            arrowLinkPopup.add(new GotoLinkNodeAction(link.getSource()
                    .toString(), link.getSource()));
            arrowLinkPopup.add(new GotoLinkNodeAction(link.getTarget()
                    .toString(), link.getTarget()));
            ...
            for (int i = 0; i < links.size(); ++i) {
                ...
                if (NodeAlreadyVisited.add(foreign_link.getTarget())) {
                    arrowLinkPopup.add(new GotoLinkNodeAction(foreign_link
                            .getTarget().toString(), foreign_link.getTarget()));
                }
                if (NodeAlreadyVisited.add(foreign_link.getSource())) {
                    arrowLinkPopup.add(new GotoLinkNodeAction(foreign_link
                            .getSource().toString(), foreign_link.getSource()));
                }
            }
    ...
    protected class ExportToHTMLAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (c.getModel().saveHTML(
                    (MindMapNodeModel) c.getModel().getRoot(), file)) {
    ...
    protected class ExportBranchToHTMLAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            try {
                ...
                if (c.getModel().saveHTML((MindMapNodeModel) getSelected(),
                        file)) {
                    loadURL(file.toString());
                }
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                if (getModel().getFile() != null) {
                    try {
                        ...
                        map.setLink(node, Tools.toRelativeURL(f.toURL(),
                                getModel().getFile().toURL()));
                    } catch (MalformedURLException ex) {
                    }
                }
                map.save(f);
                ...
                try {
                    String linkString = Tools.toRelativeURL(getModel()
                            .getFile().toURL(), f.toURL());
    ...
    private class ImportBranchAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                try {
                    MindMapNodeModel node = getModel().loadTree(
    ...
    private class NodeColorAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Color color = JColorChooser.showDialog(getView().getSelected(),
                    getFrame().getInternationalization("choose_node_color"), getSelected().getColor());
            if (color == null) {
                return;
            }
    ...
    private class EdgeColorAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            Color color = JColorChooser.showDialog(getView().getSelected(),
                    getFrame().getInternationalization("choose_edge_color"), getSelected().getEdge().getColor());
            if (color == null)
                return;
            for (ListIterator it = getSelecteds().listIterator(); it.hasNext();) {
                MindMapNodeModel selected = (MindMapNodeModel) it.next();
    ...
    private class CloudColorAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Color color = JColorChooser.showDialog(getView().getSelected(),
                    getFrame().getInternationalization("choose_cloud_color"), selectedColor);
            if (color == null)
                return;
            for (ListIterator it = getSelecteds().listIterator(); it.hasNext();) {
                MindMapNodeModel selected = (MindMapNodeModel) it.next();
    ...
    protected class ColorArrowLinkAction extends AbstractAction {
        ...
        public ColorArrowLinkAction(MindMapNode source,
                MindMapArrowLinkModel arrowLink) {
            super(getText("arrow_link_color"), new ImageIcon(
                    getResource("images/Colors24.gif")));
            this.source = source;
    ...
    protected class RemoveArrowLinkAction extends AbstractAction {
        ...
        public RemoveArrowLinkAction(MindMapNode source,
                MindMapArrowLinkModel arrowLink) {
            super(getText("remove_arrow_link"), new ImageIcon(
                    getResource("images/edittrash.png")));
            this.source = source;
            this.arrowLink = arrowLink;
        }
    ...
    protected class ChangeArrowsInArrowLinkAction extends AbstractAction {
        ...
        public ChangeArrowsInArrowLinkAction(String text, String iconPath,
                MindMapNode source, MindMapArrowLinkModel arrowLink,
                boolean hasStartArrow, boolean hasEndArrow) {
            super("", iconPath != null ? new ImageIcon(getResource(iconPath))
                    : null);
            this.source = source;
            this.arrowLink = arrowLink;
        ...
        public void actionPerformed(ActionEvent e) {
            getModel().changeArrowsOfArrowLink(source, arrowLink,
    ...
    private class NodeGeneralAction extends AbstractAction {
        ...
        NodeGeneralAction(String textID, String iconPath,
                SingleNodeOperation singleNodeOperation) {
            super(getText(textID), iconPath != null ? new ImageIcon(
                    getResource(iconPath)) : null);
    ...
    public void increaseEdgeWidth() {
        for (ListIterator e = getSelecteds().listIterator(); e.hasNext();) {
            ...
            if (currentEdgeWidthArrayIndex < edgeWidths.length - 1)
                getModel().setEdgeWidth(selected,
                        edgeWidths[currentEdgeWidthArrayIndex + 1]);
        }
    }
    ...
    public void decreaseEdgeWidth() {
        for (ListIterator e = getSelecteds().listIterator(); e.hasNext();) {
            ...
            int currentEdgeWidth = ((MindMapNodeModel)selected).getWidthWithParentInformation();
            int currentEdgeWidthArrayIndex = getEdgeWidthInEdgeWidths(currentEdgeWidth);
            if (currentEdgeWidthArrayIndex > 0)
                getModel().setEdgeWidth(selected,
                        edgeWidths[currentEdgeWidthArrayIndex - 1]);
    ...
    private int getEdgeWidthInEdgeWidths(int edgeWidth) {
        ...
        switch (edgeWidth) {
        // switch does not even accept final array access => using values
        case -1:
        ...
        case 0:
        	...
        	break;
        case 1:
            edgeWidthArrayIndex = 1; // thin and thickness 1 are equal
        	break;
        case 2:
    ...
    public void updateActions() {
        if (isEditable()) {
            ...
            increaseFontSizeAction = new NodeGeneralAction(
            ...
            decreaseFontSizeAction = new NodeGeneralAction(
            ...
            italicAction = new NodeGeneralAction("italic",
                    "images/Italic24.gif", new SingleNodeOperation() {
                        public void apply(MindMapMapModel map,
                                MindMapNodeModel node) {
                            map.setItalic(node);
                        }
                    });
            boldAction = new NodeGeneralAction("bold", "images/Bold24.gif",
            ...
            cloudAction = new NodeGeneralAction("cloud", "images/Cloud24.gif",
                    new SingleNodeOperation() {
                        ...
                        public void apply(MindMapMapModel map,
                                MindMapNodeModel node) {
                            // store last color to enable if the node is
                            // switched on and
                            // off.
            ...
            edgeWidthParentAction = new EdgeWidthAction(

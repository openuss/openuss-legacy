...
/**
 * The one and only instance of this class - the users manager - managages the relevant data of
 * the users. These are the login name, password, allowed user roles, current user role and the
 * directory name of the users data. In this directory the learners data of the learning units are stored (e.g. the personal
 * notes, the own chapters of the text study, etc.) TO DO: Save the xml file when closing the
 * program so that the current user role is saved.
 * @author Norman Lahme
 * @version 1.0
 */
// Carsten Fiedler 29.08.2005
public class FSLUsersManager implements FSLUserEventGenerator, FSLProgramConfigurationListener {
    private final static String LEARNER_ROLE = "learner";
    ...
    // Carsten Fiedler, 19.03.2007
    // the directories are defined in the config.xml
    public void init(FSLProgramConfigurationEventGenerator configurationEventGenerator,
        FSLUserListener userListener, File usersDirectory) {
            ...
            internationalization = new FLGInternationalization("freestyleLearning.homeCore.usersManager.internationalization",
                FSLUsersManager.class.getClassLoader());
    ...
    public void saveUserFrameConfiguration() {
        if (currentUserDescriptor != null && currentUserDescriptor.getRememberFrameStatusEnabled()) {
            ...
            if (!currentUserDescriptor.getUserFrameOptions().isEmpty()) 
                currentUserDescriptor.getUserFrameOptions().clear();
    ...
    // Listener methods for ConfigurationListener
    public void configurationChanged(FSLProgramConfigurationEvent event) {
        if (event.getEventType() == FSLProgramConfigurationEvent.AUTOMATIC_LOGIN) {
            ...
            if (userDescriptor != null) {
                ...
                userEvent = freestyleLearning.homeCore.usersManager.event.FSLUserEvent.createUserChangedEvent(
                    new File(usersDirectory, currentUserDescriptor.getLearningUnitsUserDataDirectoryName()),
                    currentUserDescriptor.getUserName(), currentUserDescriptor.getUserPassword(),
                    currentUserDescriptor.getCurrentUserRole(), colors, fontSizes, true,
                    automaticSelectionEnabled, displayWelcomeScreenEnabled, rememberFrameStatusEnabled,
                    frameWidth, frameHeight, frameLocationX, frameLocationY, frameMaximized);
                fireUserEvent(userEvent);
            }
            else {
                FLGOptionPane.showMessageDialog(internationalization.getString("message.userUnknown"),
                    internationalization.getString("message.loginFailure"), FLGOptionPane.ERROR_MESSAGE);
            }
        }
        ...
        if (event.getEventType() == FSLProgramConfigurationEvent.FONT_CHANGED) {
            if (currentUserDescriptor != null) {
                ...
                fontDescriptor.setElementsContentsPanelFontSize("" +
                    UIManager.get("FSLLearningUnitViewElementsContentsPanel.BaseFontSize"));
                fontDescriptor.setStructureTreeFontSize("" +
                    UIManager.get("FSLLearningUnitViewElementsStructurePanel.BaseFontSize"));
...
//        menuItem_newUser.setAccelerator(KeyStroke.getKeyStroke(newUserChar, KeyEvent.CTRL_MASK));
        ...
        buttonGroup.add(menuItem);
        /**
        menuItem = createMenuItem("menu.user.changeUserRole.administrator.title", true);
        subMenu.add(menuItem);
        buttonGroup.add(menuItem);
        **/
    ...
    private void buildDependentUI() {
        if (currentUserDescriptor != null) {
            ...
            if (currentUserDescriptor.getAllowedUserRoles().size() > 1) {
                ...
                //getAction("menu.user.changeUserRole.administrator.title").setEnabled(userRoleIsAllowed(ADMINISTRATOR_ROLE));
                ...
                //((JCheckBoxMenuItem)getMenuItem("menu.user.changeUserRole.administrator.title")).setSelected(currentUserRole.equals(ADMINISTRATOR_ROLE));
            }
            else {
        ...
        if (!vetoableEvent.isVeto()) {
            ...
            if (userLoginDialog.showDialog()) {
                ...
                if (userDescriptor != null) {
                	...
                	try {
                		if (userDescriptor.getUserPassword().equals("author")
                				|| userDescriptor.getUserPassword().equals("learner")
                				|| userDescriptor.getUserPassword().equals("admin")) {
                			password_hash = true;
                		} else {
                    ...
                    if (password_hash) {
                        ...
                        FLGOptionPane.showMessageDialog(internationalization.getString("message.passwordIncorrect"),
                            internationalization.getString("message.loginFailure"), FLGOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    FLGOptionPane.showMessageDialog(internationalization.getString("message.userUnknown"),
    ...
    private void performUserLogin(FSLUserDescriptor userDescriptor) {
        ...
        // done so far, trigger reactions to user login
        userEvent = freestyleLearning.homeCore.usersManager.event.FSLUserEvent.createUserChangedEvent(
            new File(usersDirectory, currentUserDescriptor.getLearningUnitsUserDataDirectoryName()),
            currentUserDescriptor.getUserName(), currentUserDescriptor.getUserPassword(),
            currentUserDescriptor.getCurrentUserRole(), colors, fontSizes, userLoginDialog.isAutomaticLoginEnabled(),
            automaticSelectionEnabled, displayWelcomeScreenEnabled, rememberFrameStatusEnabled, 
            frameWidth, frameHeight, frameLocationX, frameLocationY, frameMaximized);
        fireUserEvent(userEvent);
    }
    ...
    private void showNewUserDialog() {
        ...
        if (newUserDialog.showDialog()) {
            if (openUssUserCheckPerformed(newUserDialog)) {
                if (newUserDialog.getUserName() != null && newUserDialog.getUserName().length() > 0 &&
                    newUserDialog.getPassword() != null && newUserDialog.getPassword().length() > 0 &&
                    newUserDialog.isPasswordConfirmed()) {
                        String userName = newUserDialog.getUserName();
                        if (!checkUserExisting(userName)) {
                        	
                        	// get password and enrypt before storing
                            ...
                            FSLUserDescriptor newUserDescriptor =
                                FSLUserDescriptor.createFSL_UserDescriptor(createUserId(userName), userName, password,
                                newUserDialog.getUserRoles());
                            usersDescriptor.getUsersDescriptors().add(newUserDescriptor);
                            saveUsersDescriptor();
                            performUserLogin(newUserDescriptor);
                        }
                        else {
                            FLGOptionPane.showMessageDialog(internationalization.getString("message.userExists.text"), 
                                internationalization.getString("message.userExists.title"), FLGOptionPane.WARNING_MESSAGE);
                        }
                }
            }
            else {
                FLGOptionPane.showMessageDialog(internationalization.getString("message.failed.text"), 
                    internationalization.getString("message.failed.title"), FLGOptionPane.WARNING_MESSAGE);
            }
        }
    }
    ...
    private void showDeleteUserDialog() {
        if (currentUserDescriptor != null) {
            if (FLGOptionPane.showConfirmDialog(internationalization.getString("dialog.deleteUser.message1")
                + " \"" + currentUserDescriptor.getUserName() + "\"\n" + internationalization.getString("dialog.deleteUser.message2"),
                internationalization.getString("dialog.deleteUser.title"), FLGOptionPane.YES_NO_OPTION, 
    ...
    private boolean openUssUserCheckPerformed(FSLNewUserDialog dialog) {
        if (!(dialog.getUserName() != null && dialog.getUserName().length() > 0 &&
            dialog.getPassword() != null && dialog.getPassword().length() > 0 &&
            dialog.isPasswordConfirmed())) {
                return false;
        }
        ...
        StringBuffer urlBuffer = new StringBuffer("http://" + openUssServerName  
            + "/extension/directaccess/registration/RegistrationAction.po"
            ...
            + "&RetypePassword=" + dialog.retypePassword
            + "&EMail=" + dialog.email
            + "&LanguageId=" + dialog.language_id);   
...
//        JFrame frame = new JFrame(internationalization.getString("responseFrame.title.text"));
//        displayResponsePane = new FLGHtmlPane();
//        URL baseUrl = null;
//        try {
//            baseUrl = new URL("http://" + openUssServerName  + "/extension/directaccess/registration");		
//        } 
//        catch (MalformedURLException e) {}        
//        displayResponsePane.setBase(baseUrl);
//        displayResponsePane.setText(internationalization.getString("text.pleaseWait"));
//        Dimension screenDim = Toolkit.getDefaultToolkit().getScreenSize();
//        double frameScaleFactor = 0.70;
//        int frameHeight = (int)(screenDim.height*frameScaleFactor);
//        int frameWidth = (int)(screenDim.width*frameScaleFactor);
//        frame.setLocation((screenDim.width - frameWidth)/2, (screenDim.height - frameHeight)/2);
//        frame.setSize(frameWidth, frameHeight);
//        frame.getContentPane().setLayout(new BorderLayout());
//        frame.getContentPane().add(new JScrollPane(displayResponsePane));
//        frame.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
//        frame.show();        
//        Thread displayResponseThread = new Thread(new Runnable() {
//            public void run() {
//                displayResponsePane.setText(response);
//            }
//        });
//        displayResponseThread.start();

        if (response != null && FLGUtilities.contains(response, "OK: Success")) {
            FLGOptionPane.showMessageDialog(internationalization.getString("message.success.text"), 
                internationalization.getString("message.success.title"), FLGOptionPane.OK_OPTION);        
            return true;
        }
        return false;
    }
    ...
    private void setCurrentUserRole(String currentUserRole) {
        ...
        FSLUserEvent event = FSLUserEvent.createUserRoleChangedEvent(currentUserDescriptor.getUserName(),
            currentUserDescriptor.getCurrentUserRole());
        fireUserEvent(event);
    }
    ...
    private void menuItemSelected(String menuItemId) {
        ...
        if (menuItemId == "menu.user.changeUserRole.learner.title")
            setCurrentUserRole(LEARNER_ROLE);
        if (menuItemId == "menu.user.changeUserRole.author.title") setCurrentUserRole(AUTHOR_ROLE);
        /**
        if (menuItemId == "menu.user.changeUserRole.administrator.title")
            setCurrentUserRole(ADMINISTRATOR_ROLE);
            **/
    }
}
